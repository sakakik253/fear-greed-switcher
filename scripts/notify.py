...
def detect_events(dashboard: dict) -> list[dict]:
    ...
    sma_now, sma_prev = s["sma200"][i], s["sma200"][j]
    if sma_now and sma_prev:
        bull_now, bull_prev = s["btc"][i] > sma_now, s["btc"][j] > sma_prev
        if bull_now != bull_prev:
            events.append(
                {
                    "title": "BTC が 200 日移動平均を" + ("上抜け（上げ相場判定）" if bull_now else "下抜け（下げ相場判定）"),
                    "message": f"BTC {s['btc'][i]:,.0f} USD、200 日移動平均 {sma_now:,.0f} USD。",
                    "priority": 3,
                    "tags": ["compass"],
                }
            )
